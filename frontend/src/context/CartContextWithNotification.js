...
export const CartProvider = ({ children }) => {
  ...
  const addToCart = async (product, quantity = 1) => {
    ...
    try {
      if (state.isGuest) {
        ...
        return { success: true, message: 'Đã thêm sản phẩm vào giỏ hàng', product, quantity };
      } else {
  ...
  const removeFromCart = async (productId) => {
    try {
      ...
      const productItem = state.items.find(item => item.id === productId);
      if (productItem) {
  ...
  const getShippingFee = () => {
    return state.items.length > 0 ? state.shippingFee : 0;
  };
